...
import xworld_navi_easy_goal, networks
from learning import async_actor_critic, actor_critic, cuda
import time
import logging
...
def train(rank, args, shared_model, optimizer):
    ...
    model = networks.Network(height, width, channel, num_hidden, num_actions)
    model = model.cuda() if cuda.use_cuda() else model

    solver = async_actor_critic.AsyncActorCritic(args.gamma, model, shared_model, optimizer)
...
def main():
    ...
    num_hidden = 128
    num_actions = xworld.agent.num_actions
    model = networks.Network(height, width, channel, 128, num_actions)
    model = model.cuda() if cuda.use_cuda() else model
    model.share_memory()
    ...
    if args.train:
        ...
        solver = actor_critic.ActorCritic(args.gamma, model)
